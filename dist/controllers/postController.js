...
exports.getPostAnalytics = exports.deletePost = exports.getPostById = exports.getPosts = exports.updatePost = exports.createPost = void 0;
...
const updatePost = async (req, res) => {
    ...
    // ✅ Find post
    const post = await postModel_1.Post.findById(id);
    if (!post)
    ...
    if (updateData.platform)
        post.platform = updateData.platform;
...
const getPosts = async (req, res) => {
    const { page = "1", limit = "20", status, platform, sort = "-createdAt", search, startDate, endDate, } = req.query;
    ...
    const posts = await postModel_1.Post.find(filter)
        .sort(sortOption)
...
const getPostById = async (req, res) => {
    ...
    const post = await postModel_1.Post.findOne(filter)
...
const deletePost = async (req, res) => {
    ...
    // ✅ Find the post first
    const post = await postModel_1.Post.findOne(filter).lean();
    if (!post) {
    ...
    // ✅ Delete post
    const deletedPost = await postModel_1.Post.deleteOne({ _id: id });
...
const getPostAnalytics = async (req, res) => {
    try {
        ...
        // 2️⃣ Fetch Post
        const post = await postModel_1.Post.findById(postId).lean();
        if (!post)
            return res.status(404).json({ message: "Post not found" });
        // 3️⃣ Fetch Engagement Data for this Post
        const engagements = await engagementModel_1.default.find({ postId }).lean();
        ...
        // Aggregate metrics
        // Aggregate metrics
        const totals = engagements.reduce((acc, e) => {
        ...
        const totalEngagement = totals.likes + totals.comments + totals.shares + totals.clicks; // include clicks
        const engagementRate = totals.impressions > 0 ? (totalEngagement / totals.impressions) * 100 : 0;
        ...
        const performanceScore = engagementRate * 0.4 + clickThroughRate * 0.3 + totals.shares * 0.3;
        const analyticsData = {
            totalEngagement,
            engagementRate,
            clickThroughRate,
        ...
        await analyticsCache_1.default.findOneAndUpdate({ cacheKey }, // find by cacheKey
        {
            cacheKey,
            userId,
            data: analyticsData,
