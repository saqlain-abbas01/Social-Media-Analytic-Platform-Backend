...
function getEngagementMultiplier() {
  ...
  if (hour >= 9 && hour <= 17) timeMultiplier = 1.5;
  else if (hour >= 18 && hour <= 22) timeMultiplier = 1.2;
...
function getAgeMultiplier(publishedAt: Date) {
  const hoursSincePublished =
    (Date.now() - new Date(publishedAt).getTime()) / (1000 * 60 * 60);
  if (hoursSincePublished < 12) return 1.5;
  if (hoursSincePublished < 48) return 1.2;
  if (hoursSincePublished < 120) return 0.8;
  return 0.5;
}
...
async function simulateEngagement() {
  try {
    ...
    const engagementsToInsert = publishedPosts.map((post) => {
      const multiplier =
        getEngagementMultiplier() * getAgeMultiplier(post.publishedAt!);
    ...
    await Engagement.insertMany(engagementsToInsert);
    console.log(
      `✅ [Cron] Simulated ${engagementsToInsert.length} engagements`
    );
  } catch (err) {
